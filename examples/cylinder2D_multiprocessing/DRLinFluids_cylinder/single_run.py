...
def get_args():
    ...
    parser.add_argument(
        '--imitation-hidden-sizes', type=int, nargs='*', default=[512, 128]
    )
    ...
    parser.add_argument(
        '--device', type=str, default='cuda' if not torch.cuda.is_available() else 'cpu'
    )
    parser.add_argument(
        "--logger",
        type=str,
...
def test_sac_with_il(args=get_args()):
    ...
    env_step = 400
    result = collector.collect(n_step=env_step,n_episode=None, )
    rews, lens = result["rews"], result["lens"]
